...
//////////////////////////////////////////////////
//>>>DAVEX:REWIND options menu
//////////////////////////////////////////////////


extern int rewind_replay_delay; // davex:replay delay in milisecs
extern int rewind_save_rate;// davex:save rate in frames 
menu_entry opt9_entries[] =
{
	{ "Save rate (in frames)",          	MB_RANGE, MA_OPT9_REWIND_SAVE_RATE,     &rewind_save_rate, 0, 10, 120, 1 },
	{ "Replay delay (in milisecs)",       MB_RANGE, MA_OPT9_REWIND_REPLAY_DELAY,     &rewind_replay_delay, 0, 100, 255, 1 }, //davex: 255 don't care
	{ "done",                      				MB_NONE,  MA_OPT9_DONE,           NULL, 0, 0, 0, 1 },
};

#define OPT9_ENTRY_COUNT (sizeof(opt9_entries) / sizeof(opt9_entries[0]))


static void draw_rewind_menu_options(int menu_sel)
{
	int tl_x = 80+25, tl_y = 16+50;
	...
	me_draw(opt9_entries, OPT9_ENTRY_COUNT, tl_x, tl_y, NULL, NULL);
	
	menu_draw_end();
}

static void rewind_menu_loop_options(void)
{
	static int menu_sel = 0;
	int menu_sel_max;
	unsigned long inp = 0;
	...
	menu_sel_max = me_count_enabled(opt9_entries, OPT9_ENTRY_COUNT) - 1;

	for(;;)
	{
		draw_rewind_menu_options(menu_sel);
		inp = wait_for_input(BTN_UP|BTN_DOWN|BTN_LEFT|BTN_RIGHT|BTN_X|BTN_CIRCLE, 0);
		if (inp & BTN_UP  ) { menu_sel--; if (menu_sel < 0) menu_sel = menu_sel_max; }
		if (inp & BTN_DOWN) { menu_sel++; if (menu_sel > menu_sel_max) menu_sel = 0; }
		selected_id = me_index2id(opt9_entries, OPT9_ENTRY_COUNT, menu_sel);
		
		
		if (inp & (BTN_LEFT|BTN_RIGHT)) { // multi choise
				
				switch (selected_id) {
					case MA_OPT9_REWIND_SAVE_RATE: 
						 if (inp & BTN_RIGHT) {
						 	rewind_save_rate += 5;if (rewind_save_rate > 120)rewind_save_rate = 120;	
						 } else { rewind_save_rate -=5; if( rewind_save_rate < 5) rewind_save_rate = 5;
						 }
						 break;	 	 
					case MA_OPT9_REWIND_REPLAY_DELAY: 
						if (inp & BTN_RIGHT) {
						 	 rewind_replay_delay += 50; if( rewind_replay_delay > 1000) rewind_replay_delay = 1000;
						  } else { rewind_replay_delay -= 50; if (rewind_replay_delay < 50) rewind_replay_delay = 50;
						 }
						 break;	 
					default:
...
menu_entry opt_entries[] =
{
	...
	{ "[Rewind options]",          MB_NONE,  MA_OPT_REWIND_OPTS,   NULL, 0, 0, 0, 1 }, //davex
			...
			if (!me_process(opt_entries, OPT_ENTRY_COUNT, selected_id, 1))
			{
				switch (selected_id)
				{
					...
					case MA_OPT_ADV_OPTS:
						amenu_loop_options();
						break;
					case MA_OPT_REWIND_OPTS: //davex
						rewind_menu_loop_options();
						break;
